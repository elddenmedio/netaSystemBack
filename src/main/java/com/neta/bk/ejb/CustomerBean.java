/**
 * 
 */
package com.neta.bk.ejb;

import java.util.ArrayList;
import java.util.List;

import javax.ejb.EJB;
import javax.ejb.Stateless;
import javax.persistence.EntityManager;
import javax.persistence.NoResultException;
import javax.persistence.PersistenceContext;
import javax.persistence.Query;
import javax.validation.ConstraintViolation;
import javax.validation.ConstraintViolationException;

import org.json.JSONObject;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import com.neta.bk.ejb.local.CustomerLocal;
import com.neta.bk.ejb.local.LoginLocal;
import com.neta.bk.entity.CustomerEntity;
import com.neta.bk.entity.CustomerInfoEntity;
import com.neta.bk.entity.LoginEntity;
import com.neta.bk.entity.TokenEntity;
import com.neta.bk.model.LoginModel;
import com.neta.bk.model.RegisterModel;

/**
 * @author elddenmedio
 *
 */

@Stateless(name="CustomerLocal")
public class CustomerBean implements CustomerLocal {
	private static Logger LOGGER = LoggerFactory.getLogger(CustomerBean.class);
	
	@PersistenceContext
	private EntityManager entityManager;
	
	@EJB(name = "LoginLocal")
	LoginLocal loginLocal;
	
	@Override
	public JSONObject findAllCustomer() {
		Query query = entityManager.createNamedQuery(CustomerEntity.findAll);
		
		List<CustomerEntity> listResult = (List<CustomerEntity>) query.getResultList();

		JSONObject resp = new JSONObject();
		listResult.forEach(element -> {
			resp.put("id", element.getId());
			resp.put("name", element.getName());
			resp.put("surname", element.getSurname());
			resp.put("secondsurname", element.getSecondsurname());
			resp.put("active", element.getActive());
			resp.put("deleted", element.getDeleted());
			resp.put("birthday", element.getCustomer_info().getBirthday());
			resp.put("avatar", element.getCustomer_info().getAvatar());
		});
		
		return resp;
	}
	
	@Override
	public CustomerEntity findCustomerByID(Long customerID) {
		return entityManager.find(CustomerEntity.class, customerID);
	}
	
	@Override
	public JSONObject findCustomerByMail(String mail) {
		Query query = entityManager.createNamedQuery(CustomerEntity.findByMail)
														.setParameter("mail", mail);
		
		CustomerEntity result = (CustomerEntity) query.getSingleResult();

		JSONObject resp = new JSONObject();
		resp.put("id", result.getId());
		resp.put("name", result.getName());
		resp.put("surname", result.getSurname());
		resp.put("secondsurname", result.getSecondsurname());
		resp.put("active", result.getActive());
		resp.put("deleted", result.getDeleted());
		resp.put("birthday", result.getCustomer_info().getBirthday());
		resp.put("avatar", result.getCustomer_info().getAvatar());
		
		return resp;
	}
	
	@Override
	public CustomerEntity createCustomer(RegisterModel register) {
		try {
			CustomerEntity customer = findOrCreate(register.getMail());
			//customer.setId(Long.valueOf(120));
			customer.setName(register.getName());
			customer.setSurname(register.getSurname());
			customer.setSecondsurname(register.getSecondsurname());
			customer.setActive(false);
			customer.setDeleted(false);
			customer.setCustomer_info(createCustomerInfo(register));
			customer.setLogin(createLogin(register));
			
			CustomerEntity entity = saveCustomer(customer);
			
			return entity;
		} catch(NoResultException e) {
	        return null;
	    }
	}
	
	public CustomerEntity activateCustomer(Long customerID) {
		try {
			CustomerEntity entity = entityManager.find(CustomerEntity.class, customerID);
			entity.setActive(true);
			
			return saveCustomer(entity);
		} catch(NoResultException e) {
			return null;
	    }
	}
	
	@Override
	public JSONObject validatedLogin(LoginModel login) throws Exception {
		LoginEntity entity = loginLocal.validateLogin(login);
		
		return getCustomerLoginInfo(entity);
	}
	
	@Override
	public JSONObject getCustomerLoginInfo(LoginEntity entity) {
		return mapCustomerInfo(entity);
	}
	
	@Override
	public JSONObject getCustomerRegisterInfo(TokenEntity tokenEntity, String token) throws Exception {
		LoginModel login = new LoginModel();
		login.setUsername(tokenEntity.getCustomer().getLogin().getUsername());
		login.setPassword(tokenEntity.getCustomer().getLogin().getPassword());
		
		LoginEntity entity = loginLocal.validateLogin(login);
		
		return mapCustomerInfo(entity);
	}
	
	private JSONObject mapCustomerInfo(LoginEntity entity) {
		JSONObject json = new JSONObject();
		
		json.put("id", entity.getCustomer().getId());
		json.put("token", entity.getCustomer().getTokens().get(0).getToken());
		json.put("name", entity.getCustomer().getName());
		json.put("surname", entity.getCustomer().getSurname());
		json.put("secondsurname", entity.getCustomer().getSecondsurname());
		json.put("username", entity.getCustomer().getCustomer_info().getMail().split("@")[0]);
		json.put("mail", entity.getCustomer().getCustomer_info().getMail());
		json.put("avatar", "data:image/png;base64,".concat(entity.getCustomer().getCustomer_info().getAvatar()));
		json.put("dateUp", entity.getCustomer().getTokens().get(0).getDate_created());
		json.put("dateBirthDay", entity.getCustomer().getCustomer_info().getBirthday());
		
		return json;
	}
	
	private CustomerEntity findOrCreate(String mail) {
		try {
			Query query = entityManager.createNamedQuery(CustomerEntity.findByMail)
															.setParameter("mail", mail);
			
			CustomerEntity entity = (CustomerEntity) query.getSingleResult();
			
			return entity;
		} catch(NoResultException e) {
			return new CustomerEntity();
	    }
	}
	
	private CustomerInfoEntity createCustomerInfo(RegisterModel register) {
		CustomerInfoEntity entity = new CustomerInfoEntity();
		entity.setAvatar("");
		
		return entity;
	}
	
	private LoginEntity createLogin(RegisterModel register) {
		LoginEntity entity = new LoginEntity();
		String[] username = register.getMail().split("@");
		entity.setUsername(username[0]);
		entity.setPassword(register.getPassword());
		
		return entity;
	}
	
	private CustomerEntity saveCustomer(CustomerEntity entity) {
		try {
			entityManager.persist(entity);
			entityManager.flush();
	    } catch (ConstraintViolationException e) {
	        for (ConstraintViolation<?> actual : e.getConstraintViolations()) {
	            LOGGER.info(actual.toString());
	        }
	    }
		
		return entity;
	}
}
